...
`hyperfine --runs 3 --warmup 1 'npx eslint configs src'`
...
| 2025-06-07#3 | 2.1s  | 22.15 | romain duc win11  | using `npx biome check configs src`                           |
| 2025-06-07#4 | 0.3s  | 22.15 | romain duc win11  | using `bunx biome check configs src` bye bye `eslint` & `npx` |

Note 1 : to show time taken by rules : `TIMING=1 npx eslint configs src`
...
Note 3 : to list eslint scanned files : `DEBUG=eslint:cli-engine npx eslint configs src`
