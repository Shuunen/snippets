document.querySelectorAll('.stable-body > table > tbody > tr[index][title]').forEach(row => {
  ...
  let nbMo = 0
  if (sentUnit === 'Kio') nbMo = sentValue / 1000
  else if (sentUnit === 'Mio') nbMo = sentValue
  else if (sentUnit === 'Gio') nbMo = sentValue * 1000
  else if (sentUnit === 'Tio') nbMo = sentValue * 1000 * 1000
  else return console.warn(name, 'has an unhandled unit :', sentUnit)
  const dateElement = row.querySelector('.stable-List-col-14 div')
  if (!dateElement) return console.warn('failed to find a date on', name)
