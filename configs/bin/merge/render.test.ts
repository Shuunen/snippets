...
import type { BySide, FileDetails, Side, SyncFile } from '../types'
...
describe('merge render', () => {
  ...
  it('buildPanel previews the winning text on both sides once a choice is pending', () => {
    const rows = panelOf('repo').map(row => stripAnsi(row))
    expect(rows.some(row => row.includes('B'))).toBe(false)
  })
