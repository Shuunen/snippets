import { type BySide, type Side, sides, type SyncFile } from '../core/types'
import { homeDir } from '../sync/catalog'
import type { Block } from './blocks'
import { type CharSpan, computeCharDiffSpans, computeMergedLineSegments } from './char-diff'
 ...
 * Both sides always show their own text — see `resolveBlockPreview` — so once a side is pending, the
 * side it would overwrite still reads as itself : a modified line renders as a merge of its own
 * words (struck through) and the words replacing them (plain), so both what's leaving and what's
 * arriving stay visible ; a line dropped entirely (the overwrite shrinks that side) stays on screen
 * fully struck through instead of vanishing.
...
function computeConflictRows(block: Block, options: ConflictOptions): ConflictRow[] {
  ...
  // a fully deleted line is struck through as a whole rather than merged, so its spans are never consulted
  const spans: BySide<CharSpan[]> = resolveDisplayKind(block, pending) === 'modification' ? computeCharDiffSpans(block.text) : { local: [], repo: [] }
  const rowsFor = (side: Side, lineIndex: number, kind: BlockChangeKind | undefined): string[] => {
    ...
    // this side is about to be overwritten by a modification : merge its own words with the winning side's, in place
    const winningLine = pending && side !== pending && kind === 'modification' ? preview[pending][lineIndex] : undefined
    const lineOptions = winningLine
      ? { language, merged: computeMergedLineSegments(line.text, winningLine.text), width, wrapEnabled }
      : { changeSpans: spans[side].filter(span => span.line === lineIndex), deleted: line.isDeleted, language, width, wrapEnabled }
    return prepareConflictLine(line.text, lineOptions).map(row => `${gutter}${row}`)
  }
