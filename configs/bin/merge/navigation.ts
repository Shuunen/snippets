...
/**
 * Resolve the lines each side should display given a pending choice. Both sides always keep
 * showing their own lines — the overwritten side never switches to the winning text, so exactly
 * which characters it's about to lose can still be struck through in place (see `render.ts`)
 * instead of the whole line silently flipping to the other side's words. The one exception is a
 * side losing more lines than it gains : the surplus beyond what the winning side offers stays on
 * screen flagged as deleted, so it reads as struck through rather than just vanishing. That covers
 * both a side wiped out entirely and a modification that simply shrinks it, e.g. two lines
 * replaced by one.
 * @param block the block being resolved
 * @param pending the side currently chosen to win, if any
...
export function resolveBlockPreview(block: Block, pending: Side | undefined): BySide<PreviewLine[]> {
  ...
  const ownLines = lines[otherSide(pending)]
  const overlapCount = Math.min(ownLines.length, winning.length)
  const overlapping = ownLines.slice(0, overlapCount).map(line => kept(line))
  const growth = winning.slice(overlapCount)
  const surplus = ownLines.slice(overlapCount).map(line => dropped(line))
  const overwritten = [...overlapping, ...growth, ...surplus]
