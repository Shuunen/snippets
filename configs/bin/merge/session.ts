...
import { writeAtomic } from '../sync/paths'
...
async function runOneAttempt(task: FileTask, session: MergeSession): Promise<'abort' | 'external' | 'resolved' | 'skipped'> {
  ...
  // written one side at a time (never Promise.all) so an interrupt between them can never leave
  // one side updated and the other stale with no way to tell after the fact
  // oxlint-disable-next-line no-await-in-loop
  for (const side of sides) await writeAtomic(file.sides[side].filepath, merged[side])
  return 'resolved'
}
