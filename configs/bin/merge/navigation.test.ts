...
describe('merge navigation', () => {
  ...
  it('resolveBlockPreview previews the incoming local text on repo when repo is overwritten by a modification', () => {
    expect(resolveBlockPreview(makeBlock('b', 'B'), 'local')).toStrictEqual({ local: [{ isDeleted: false, text: 'B' }], repo: [{ isDeleted: false, text: 'B' }] })
  })

  it('resolveBlockPreview previews the incoming repo text on local when local is overwritten by a modification', () => {
  ...
  it('resolveBlockPreview flags the surplus lines a shrinking modification would drop', () => {
  ...
  it('resolveBlockPreview flags nothing when a growing modification only adds lines', () => {
    const block = makeBlock('  "voiceEnabled": false\n', '  "voiceEnabled": false,\n  "model": "opus"\n')
    const preview = resolveBlockPreview(block, 'local')
    expect(preview.repo).toStrictEqual(preview.local)
